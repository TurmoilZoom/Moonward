...
internal class ZZZGachaService : GachaLogService
{
    ...
    /// <summary>
    /// 通过米游社/HoYoLAB 养成指南接口（icon_info + item_list）更新本地抽卡物品元数据。
    /// 国际服角色尽量拉取全部 UI 语言名称；国服通常仅中文。图标 URL 与语言无关。
    /// </summary>
    /// <param name="role">任意已登录的绝区零角色。</param>
    /// <param name="preferredLanguage">写入 <c>ZZZGachaInfo.Name</c> 时优先使用的语言（通常为当前 UI 语言）。</param>
    /// <param name="progress">进度文案（可选）。</param>
    /// <param name="cancellationToken">取消令牌。</param>
    /// <returns>
    /// Languages：写入的语言列表；
    /// ItemCount：物品数量；
    /// UsedFallback：true 表示养成接口风控/失败后改用静态元数据 CDN。
    /// </returns>
    public async Task<(IReadOnlyList<string> Languages, int ItemCount, bool UsedFallback)> UpdateGachaInfoFromGameRecordAsync(
        GameRecordRole role,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string preferred = LanguageUtil.FilterLanguage(preferredLanguage);
        bool isHoyolab = role.GameBiz?.EndsWith("_global", StringComparison.OrdinalIgnoreCase) ?? false;

        // 国际服可切换语言；国服养成接口名称通常固定中文
        List<string> languages = isHoyolab
            ? LanguageUtil.GetAllLanguages()
            : [preferred is "zh-cn" or "zh-tw" ? preferred : "zh-cn"];

        // 优先把 preferred 放到第一位，使 ZZZGachaInfo.Name 与 UI 语言一致
        if (languages.Remove(preferred))
        {
        ...
        var writtenLanguages = new List<string>();
        int itemCount = 0;
        bool wroteInfoTable = false;
        ...
        try
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                progress?.Report(string.Format(Lang.GachaLogPage_FetchingItemInfoLanguage, lang));
                ZZZGachaWiki wiki = await _gameRecordService.GetZZZGachaWikiFromCultivateToolAsync(role, lang, cancellationToken);
                if (wiki.List is null || wiki.List.Count == 0)
                {
                    continue;
                }

                string actualLang = string.IsNullOrWhiteSpace(wiki.Language) ? lang : LanguageUtil.FilterLanguage(wiki.Language);
                // 首个成功语言写入信息表（含图标）；后续语言只补名称缓存，避免反复覆盖 Name
                if (!wroteInfoTable)
                {
                    WriteZZZGachaInfoAndNameCache(wiki.List, actualLang);
                    wroteInfoTable = true;
                    itemCount = wiki.List.Count;
                }
                else
                {
                    SaveItemNamesToCache(wiki.List.Select(x => ((long)x.Id, x.Name)), actualLang);
                }
                writtenLanguages.Add(actualLang);
        ...
        {
            // 10035 等是 act 极验风控，与战绩页「验证账号」无关；回退静态元数据仍可更新图标
            _logger.LogWarning(ex, "ZZZ cultivate tool risk control ({retcode}), fallback to static metadata CDN", ex.ReturnCode);
            progress?.Report(Lang.GachaLogPage_UpdateIconsFallbackToCdn);
            string lang = await UpdateGachaInfoAsync(CurrentGameBiz, preferred, cancellationToken);
            itemCount = CountZZZGachaInfoRows();
            return ([lang], itemCount, UsedFallback: true);
        }

        if (!wroteInfoTable)
        {
            throw new InvalidOperationException(Lang.GachaLogPage_UpdateIconsNoItemData);
        }

        return (writtenLanguages, itemCount, UsedFallback: false);
    }


    /// <summary>养成/badge 接口极验风控 retcode（genshin.py GEETEST_RETCODES）。</summary>
    private static bool IsCultivateToolRiskControl(int retcode) => retcode is 10035 or 10041 or 1034 or 5003 or -3503;


    private static int CountZZZGachaInfoRows()
    {
        using var dapper = DatabaseService.CreateConnection();
        return dapper.QueryFirstOrDefault<int>("SELECT COUNT(*) FROM ZZZGachaInfo;");
    }
